...
RenderTarget::Create(filament::Engine* engine, int width, int height) {
  ...
  std::unique_ptr<RenderTarget> rt = std::make_unique<RenderTarget>();
  rt->_engine = engine;
...
void RenderTarget::Resize(int w, int h) {
  ...
  glGenTextures(1, &_tex_gl);
  glBindTexture(GL_TEXTURE_2D, _tex_gl);
  ...
  _imguiTextureId = (ImTextureID)(uintptr_t)_tex_gl;
...
filament::Texture* RenderTarget::GetColorTexture() const {
