...
Each tracking entry has spring-damper parameters defined by `PoseTrackingParams` ([C++](pathname:///generated/api/v1.0.1/cpp/structsuperdex_1_1PoseTrackingParams.html), [Python](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.PoseTrackingParams)):
...
The `PoseControllerParams` constructors ([C++](pathname:///generated/api/v1.0.1/cpp/structsuperdex_1_1PoseControllerParams.html), [Python](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.PoseControllerParams)) accept `num_links`, pre-size these arrays to the articulation's number of links, and fill them with zero-gain entries:
...
params.joint_tracking[joint_link] = sdp.PoseTrackingParams(
    stiffness=500.0,
    damping=50.0,
)
...
- Joint targets contain [`actor->GetNumDofs()`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html) values ([`get_num_dofs`](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.Actor.get_num_dofs) in Python). Translational DoFs are `[m]` and rotational DoFs are `[rad]`.
- Link targets contain one world-from-link `TransformRT` ([C++](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1TransformRT.html), [Python](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.TransformRT)) per link, including links whose tracking gains are zero.
...
[`SetArticulatedTargetPose`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html) and [`SetArticulatedTargetLinkTransforms`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html) infer target velocity from how the target changes between simulation steps. Their Python equivalents are [`set_articulated_target_pose`](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.Actor.set_articulated_target_pose) and [`set_articulated_target_link_transforms`](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.Actor.set_articulated_target_link_transforms). Use them for continuous trajectories, updating the target before each step.

[`ResetArticulatedTargetPose`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html) and [`ResetArticulatedTargetLinkTransforms`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html) set the supplied target and clear target velocity for the next step. Their Python equivalents are [`reset_articulated_target_pose`](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.Actor.reset_articulated_target_pose) and [`reset_articulated_target_link_transforms`](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.Actor.reset_articulated_target_link_transforms). Use them for discontinuous handoffs, teleports, or initialization to a target different from the pose captured when the controller was added.
...
To supply a non-zero target velocity after reset, call [`SetArticulatedTargetVelocity`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html) / [`set_articulated_target_velocity`](pathname:///generated/api/v1.0.1/python/api/physics.html#superdex.physics.Actor.set_articulated_target_velocity) after the reset. This override is consumed by the next simulation step and then cleared, so set it before every step that needs an explicit target velocity.

## Runtime Tuning

Replace the complete parameter object to change gains or turn tracking slices on and off at runtime. A freshly constructed [`PoseControllerParams(num_links)`](pathname:///generated/api/v1.0.1/cpp/structsuperdex_1_1PoseControllerParams.html) begins with every entry disabled, so filling only one slice naturally creates a joint-only or link-only configuration.
