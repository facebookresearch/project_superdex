...
- Contact dissipation (Coulomb and viscous friction, plus normal damping) is disabled
- Inertia and joint friction are disabled

The solver then calls [`Scene::Step`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Scene.html) with an infinite timestep, which reduces the equations of motion to the quasistatic equilibrium condition:
...
```python
ik_scene = sdp.create_scene("IKScene")
```
...
```python
sdp.experimental.destroy_ik_solver(ik_solver)  # also destroys ik_scene
```
...
obstacle_params = sdp.RigidActorParams()
obstacle_params.shape = sdp.BoxShape(half_extents=[0.3, 0.15, 0.3])
obstacle_params.transform = obstacle_transform
obstacle_params.body_type = sdp.BodyType.STATIC
...
**Use a separate scene for IK.** The IK solver sets the scene to infinite timestep and zero gravity, and disables contact dissipation, inertia, and joint friction. This makes the scene unusable for visualization or dynamic simulation.

**Dual-scene pattern.** Create one scene for rendering/simulation and a second scene for IK. After each `SolveIK` call, copy the resulting pose to the visualization scene with [`SetArticulatedPoseFromJoints`](pathname:///generated/api/v1.0.1/cpp/classsuperdex_1_1Actor.html). Suppress velocities on the visualization actor after copying to prevent drift.
