...
class Scheduler {
 ...
 public:
  ...
  using BatchFunction = std::function<void(int, int)>;
  ...
  // batchEnqueueOnAvailableWorkers() tries to enqueue a task on a pool of up to
  // 'targetWorkers' threads, all of them different and ready to start executing
  // the task immediately. If fewer than 'minWorkers' are available, no tasks
  // are enqueued. If 'includeSelf' is true and enough workers are available,
  // the calling thread runs the task as part of the function call. The return
  // is the number of enqueued tasks, including the task run in the calling
  // thread if 'includeSelf' is true.
  MARL_EXPORT
  int batchEnqueueOnAvailableWorkers(BatchFunction const& task,
                                     int minWorkers,
                                     int targetWorkers,
                                     bool includeSelf);
  ...
  // Protected so that users can extend the behavior by deriving from Scheduler.
  // E.g. to create alternatives to the enqueue method, with different
  // tradeoffs.
 protected:
  ...
  class Worker {
   public:
    ...
    // call unlock() or enqueueAndUnlock().
    bool tryLock() EXCLUDES(work.mutex) TRY_ACQUIRE(true, work.mutex);

    // unlock() releases the lock without enqueing any tasks.
    void unlock() REQUIRES(work.mutex) RELEASE(work.mutex);
    ...
    // isWaitingForWork() returns true if the worker is ready to start new work
    // immediately. Returns false if the worker is busy running tasks.
    bool isWaitingForWork() const;

    // steal() attempts to steal a Task from the worker for another worker.
    // If useTryLock is true, then the lock attempt will be non-blocking.
    // Returns true if a task was taken and assigned to out, otherwise false.
    bool steal(bool useTryLock, Task& out) EXCLUDES(work.mutex);
   ...
   private:
    ...
    struct Work {
      ...
      std::atomic<bool> isBusy = {true};  // True except while spinning for work
                                          // or waiting for work.
  ...
  bool stealWork(Worker* thief, uint64_t from, bool useTryLock, Task& out);
