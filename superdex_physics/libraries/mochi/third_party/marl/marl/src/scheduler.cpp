  ...
  if (cfg.workerThread.count > 0) {
    while (true) {
      ...
      if (idx >= 0) {
        ...
        // If the worker is not ready, then it must have successfully stolen a
        // task. In that case, it is not a good worker to prioritize.
        if (worker->isWaitingForWork() && worker->tryLock()) {
          if (worker->isWaitingForWork()) {
...
int Scheduler::batchEnqueueOnAvailableWorkers(BatchFunction const& task,
                                              int minWorkers,
                                              int targetWorkers,
                                              bool includeSelf) {
// Compiler warns that worker->work.mutex is is not released for every path through the loop.
// That's OK because each lock will be released in the following loop, either by calling unlock()
// or by calling enqueueAndUnlock().
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wthread-safety-analysis"
#endif

  MARL_ASSERT(minWorkers > 0 && targetWorkers >= minWorkers,
              "Inconsistent number of workers.");
  int minNonSelfWorkers = includeSelf ? (minWorkers - 1) : minWorkers;
  // TODO(T191060428): The condition below can be tightened if the calling
  // thread is a worker thread.
  if (minNonSelfWorkers > cfg.workerThread.count) {
    return 0;
  }

  // Find workers ready to start executing the task immediately.
  int targetNonSelfWorkers = includeSelf ? (targetWorkers - 1) : targetWorkers;
  std::array<int, MaxWorkerThreads> workersFound;
  int numWorkersFound = 0;
  for (int i = 0;
       i < cfg.workerThread.count && numWorkersFound < targetNonSelfWorkers;
       ++i) {
    auto* worker = workerThreads[i];
    if (worker->isWaitingForWork() && worker->tryLock()) {
      MARL_ASSERT(worker != Worker::getCurrent(),
                  "isWaitingForWork() should have returned false for the "
                  "current worker");
      if (worker->isWaitingForWork()) {
        workersFound[numWorkersFound++] = i;
      } else {
        worker->unlock();
      }
    }
  ...
  if (numWorkersFound >= minNonSelfWorkers) {
    // Enough workers are available. Enqueue and unlock.
    int selfWorkers = includeSelf ? 1 : 0;
    int numWorkers = numWorkersFound + selfWorkers;
    for (int i = 0; i < numWorkersFound; ++i) {
      int workerId = i + selfWorkers;
      workerThreads[workersFound[i]]->enqueueAndUnlock(
          Task::Function([=]() { task(workerId, numWorkers); }));
    }
    if (includeSelf) {
      task(0, numWorkers);
    }
    ...
    // Not enough workers are available. Unlock.
    for (int i = 0; i < numWorkersFound; ++i) {
      workerThreads[workersFound[i]]->unlock();
    }
    return 0;
  }
...
bool Scheduler::stealWork(Worker* thief,
                          uint64_t from,
                          bool useTryLock,
                          Task& out) {
  if (cfg.workerThread.count > 0) {
    ...
    if (thread != thief) {
      if (thread->steal(useTryLock, out)) {
        return true;
      }
    }
  }
...
void Scheduler::onBeginSpinning(int workerId) {
  // Try to add workerId to the spinningWorkers queue so that the Scheduler can
  // prioritize it when scheduling a future task.
  //
  // WARNING: This is NOT a reliable operation!
  //
  // There is no mutex to synchronize nextSpinningWorkerIdx with
  // spinningWorkers. We could add a mutex to ensure that we never forget which
  // workers are spinning, but that would significantly impact the performance
  // of Scheduler::enqueue.
  //
  auto idx = nextSpinningWorkerIdx++ % cfg.workerThread.count;
  auto displaced = spinningWorkers[idx].exchange(workerId);

  // If we accidentally removed a worker from the spinningWorkers queue, then
  // try to re-insert it. A few retries are OK but don't spend too long.
  int maxRetries = std::min(16, cfg.workerThread.count);
  for (int i = 0;
       (displaced >= 0) && (displaced != workerId) && (i < maxRetries); ++i) {
    idx = nextSpinningWorkerIdx++ % cfg.workerThread.count;
    displaced = spinningWorkers[idx].exchange(workerId);
  }
}
...
bool Scheduler::Worker::isWaitingForWork() const {
  return !work.isBusy && (work.num == 0);
}

bool Scheduler::Worker::steal(bool useTryLock, Task& out) {
  if (work.num.load() == 0) {
    return false;
  }
  if (useTryLock) {
  ...
  // Do not steal a task unless the worker is busy. If the worker is not busy
  // yet, then the task must have been newly enqueued. As soon as we release the
  // lock, that worker should see the task and start executing it. Letting tasks
  // execute on the workers to which they were originally assigned is preferred,
  // since this generally leads to a more even distribution.
...
void Scheduler::Worker::run() {
  if (mode == Mode::MultiThreaded) {
    ...
    work.isBusy = false;  // Not "busy" while waiting for work.
...
void Scheduler::Worker::waitForWork() {
  ...
  work.isBusy = false;  // Not "busy" while spinning or waiting for work.
  ...
  work.isBusy = true;  // "Busy" again, even if (work.num == 0).
...
void Scheduler::Worker::spinForWorkAndLock() {
  ...
  while (std::chrono::high_resolution_clock::now() - start < duration) {
    ...
    if (scheduler->stealWork(this, rng(), useTryLock, stolen)) {
      work.mutex.lock();
  ...
  // Don't give up until we are certain that there is no work to do. To reduce
  // lock contention, we start at a random index and iterate in reverse order,
  // with respect to the enqueue method's round-robin.
  auto workerCount = scheduler->cfg.workerThread.count;
  auto target = rng() % workerCount;
  useTryLock = false;  // Prevent spurious lock failure
  for (int i = 0; i < workerCount; i++) {
    if (scheduler->stealWork(this, target, useTryLock, stolen)) {
      work.mutex.lock();
      work.tasks.emplace_back(std::move(stolen));
      work.num++;
      return;
    ...
    target = target ? (target - 1) : (workerCount - 1);
  }
