...
// [Mochi] Members touching `mDetails` must be defined after `DracoMeshDetails`: they instantiate
// `~unique_ptr<DracoMeshDetails>`, which static_asserts on `sizeof(DracoMeshDetails)`. MSVC
// tolerates the forward declaration; clang-cl, which Mochi uses on Windows, rejects it.
#if GLTFIO_DRACO_SUPPORTED
