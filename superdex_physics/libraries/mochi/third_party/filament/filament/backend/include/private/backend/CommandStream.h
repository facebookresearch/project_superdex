...
public:
    // [Mochi] Dropped `constexpr` from this ctor. The body uses `(char*)next - (char*)this` 
    // (effectively a reinterpret_cast), which C++20 forbids in a constant expression. Clang 
    // lets it slide via -Wno-invalid-constexpr; GCC treats it as a hard error.
