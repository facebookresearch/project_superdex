...
Platform::SwapChain* PlatformWGL::createSwapChain(uint32_t width, uint32_t height, uint64_t flags) noexcept {
    ...
    // readPixels.
    // [Mochi] RECT's members are LONG, so the uint32_t extents narrow. MSVC allows this silently;
    // clang-cl rejects it as -Wc++11-narrowing, and Mochi builds Filament with clang on Windows.
