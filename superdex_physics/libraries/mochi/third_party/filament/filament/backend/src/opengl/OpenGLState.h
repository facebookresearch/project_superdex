...
public:
    ...
    // [Mochi] Dropped `constexpr` from getIndexForCap. The body calls
    // `state.enables.caps.size()` (utils::bitset::size() is noexcept but not
    // constexpr), which C++20 forbids in a constant expression. Clang lets it
    // slide via -Wno-invalid-constexpr; GCC treats it as a hard error.
    // The function is never invoked in a constant-expression context.
    inline size_t getIndexForCap(GLenum cap) noexcept;
...
// [Mochi] Dropped `constexpr` (matches the declaration above).
size_t OpenGLState::getIndexForCap(GLenum cap) noexcept { //NOLINT
    size_t index = 0;
    switch (cap) {
