...
int mochi::PerElementSoftMaterialData::GetNumElements() const {
  static_assert(
      static_cast<int>(SoftMaterialType::Count) == 6,
      "Please update the per-element count selection below if SoftMaterialType enum changes");
  switch (type) {
    case SoftMaterialType::Arap:
    case SoftMaterialType::ActiveShapeTargetingArap:
      return isize(arapStiffness);
