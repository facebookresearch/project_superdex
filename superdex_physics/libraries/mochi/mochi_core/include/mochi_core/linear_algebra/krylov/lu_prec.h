...
struct LUPrec final : Preconditioner<std::remove_const_t<Scalar>> {
  using NonConstScalar = std::remove_const_t<Scalar>;
  static constexpr auto kType = PreconditionerType::LU;

  template <typename MatrixType>
  explicit LUPrec(MatrixType const& A);
 ...
 private:
  /** @brief Prepares @p A for consumption by @ref LU. If @p A is already a dense @ref Matrix,
   * returns a dynamic-size view to avoid a copy. Otherwise, materializes it into a dense matrix via
   * @ref ToMatrix. */
  template <typename MatrixType>
  static auto ToLUInput(MatrixType const& A) {
    if constexpr (IsMatrix<MatrixType>) {
      // Create a view to avoid an unnecessary copy. Note that _lu currently requires dynamic size.
 ...
 private:
  LU<NonConstScalar> _lu;
};

template <typename Scalar>
template <typename MatrixType>
LUPrec<Scalar>::LUPrec(MatrixType const& A) : _lu(ToLUInput(A)) {
  MOCHI_ASSERT_VERBOSE(A.Rows() == A.Cols(), "Input matrix must be square.");
}

template <typename Scalar>
template <typename Input, typename Output>
void LUPrec<Scalar>::operator()(Input const& x, Output&& y) const {
  Preconditioner<NonConstScalar>::ValidateInputOutput(_lu.GetLU().Rows(), x, y);
  y = x;
  _lu.LeftSolveInPlace(y);
}
