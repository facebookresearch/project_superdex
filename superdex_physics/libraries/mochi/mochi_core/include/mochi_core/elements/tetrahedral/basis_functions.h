...
class BarycentricBasisTetrahedra final {
  ...
  */
  static constexpr real GetValue(int baseIndex, Real3 const& x) {
    // Get the barycentric coordinates
    real const sumX = x[0] + x[1] + x[2];
    Real4 const lam = {x[0], x[1], x[2], 1_r - sumX};
    return //
        GetTValue(kIndices[baseIndex][0], lam[0]) * //
        GetTValue(kIndices[baseIndex][1], lam[1]) * //
        GetTValue(kIndices[baseIndex][2], lam[2]) * //
        GetTValue(kIndices[baseIndex][3], lam[3]);
  }
  ...
  /**
    ...
    Returns :
      ndarray: 1D array of len 3 of floats containing the derivative in each
      of the directions of parametric space
  */
  static constexpr Real3 GetDValue(int baseIndex, Real3 const& x) {
    // Get the barycentric coordinates
    real const sumX = x[0] + x[1] + x[2];
    Real4 const lam = {x[0], x[1], x[2], 1_r - sumX};
    ...
    real val_l = GetTValue(kIndices[baseIndex][3], lam[3]);
    return Real3{
        GetDtValue(kIndices[baseIndex][0], lam[0]) * val_j * val_k * val_l +
            val_i * val_j * val_k * GetDtValue(kIndices[baseIndex][3], lam[3]) * (-1_r),
        val_i * GetDtValue(kIndices[baseIndex][1], lam[1]) * val_k * val_l +
            val_i * val_j * val_k * GetDtValue(kIndices[baseIndex][3], lam[3]) * (-1_r),
        val_i * val_j * GetDtValue(kIndices[baseIndex][2], lam[2]) * val_l +
            val_i * val_j * val_k * GetDtValue(kIndices[baseIndex][3], lam[3]) * (-1_r)};
  }

  static constexpr int kVertexDof = 4; // The number of vertex dof of a tetrahedron
  ...
  static constexpr int kNumDofs = // The number of degrees of freedom
      (kPolyOrder + 1) * (kPolyOrder + 2) * (kPolyOrder + 3) / 6;
  static constexpr int kEdgeDof = kPolyOrder - 1; // The number of edge dof
  static constexpr int kFaceDof = // The face number dof
      (kPolyOrder - 1) * (kPolyOrder - 2) / 2;
  static constexpr int kInteriorDof = // The number of interior dof
      (kPolyOrder - 1) * (kPolyOrder - 2) * (kPolyOrder - 3) / 6;

  // Shorthand
  using IndicesType = NdArray<int, kNumDofs, 4>;
  using DofNodesType = NdArray<real, kNumDofs, 4>;
  using OneDimKnotType = NdArray<real, kPolyOrder + 1>;

  // Compile-time algorithm to compute kIndices and kDofNodes
  static constexpr std::pair<IndicesType, DofNodesType> PrecomputeIndiciesAndNodes() {
    IndicesType indices = {};
    ...
    for (int i = 0; i < (kPolyOrder + 1); ++i) {
      for (int j = 0; j < (kPolyOrder + 1 - i); ++j) {
        for (int k = 0; k < (kPolyOrder + 1 - i - j); ++k) {
          Int4 const tet = {k, j, i, 1 - i - j - k};
          indices[count] = tet;
          dofNodes[count] = Real4{tet[0] / po, tet[1] / po, tet[2] / po, tet[3] / po};
          ++count;
        }
      }
    }
  ...
  static constexpr IndicesType kIndices = PrecomputeIndiciesAndNodes().first;
  static constexpr DofNodesType kDofNodes = PrecomputeIndiciesAndNodes().second;
